...
<body class="antialiased bg-gray-100">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
        <header class="flex flex-col md:flex-row md:justify-between md:items-center">
            ...
            <form action="{{ route('listRestaurants') }}" class="search-bar">
                <input id="search" name="search" value="{{ request()->search }}" placeholder="Rechercher un article"
                    type="search">
                <button type="submit">
            ...
            <div class="space-y-10">
                @foreach ($restaurants as $restaurant)
                    <article class="flex flex-col lg:flex-row lg:items-center pb-10 border-b border-slate-300">
                        <div class="lg:w-4/12">
                            <img class="w-full h-64 object-cover rounded-lg shadow-md" src="{{ $restaurant->thumbnail }}"
                                alt="{{ $restaurant->name }}">
                        </div>
                        <div class="lg:w-8/12 lg:pl-6 mt-4 lg:mt-0">
                            ...
                            <a href="{{ route('showRestaurant', ['restaurant' => $restaurant]) }}"
                                class="btn btn-primary mt-4 flex items-center">
                                Réserver
                            </a>
                        </div>
                    </article>
